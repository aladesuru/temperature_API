...
//Method to check if a user has signed in 
 init: () => {
   if (window.sessionStorage.getItem("token")) {              // if signed in display display temperature
    ...
    //Generate timestamp , store the timestamp and the API generated every minute in array
    // and passing the array to average method to calculate average
      $tempReading.push({timestamp : Date.now() , temp: Number(sessionStorage.temperature)});

       },60000); 

    //Calling code to produce a smaller set of data that can serve across our API
      temperature_utilities.smallerDataSet(1,5);
...
//Method to request  temperature reading
   ...
   $.ajax({
       ...
       success: function (data) {
         console.info(data); 
         sessionStorage.setItem("temperature", data.temp);  //store the temperature on client side
